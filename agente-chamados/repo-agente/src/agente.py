"""
Agente de Classificação de Chamados — v5
Integração: Google Chat + WhatsApp → Cloud Functions
"""
...
def classificar_chamado(mensagem: str, historico: list = None) -> dict:
    """Classifica o chamado via LLM e anexa diagnóstico por palavras-chave."""
    msgs = list(historico or [])
    msgs.append({"role": "user", "content": f"Analise e classifique este chamado:\n\n{mensagem}"})

    resposta = client.messages.create(
        model="claude-sonnet-4-20250514",  # modelo LLM utilizado
        max_tokens=500,
